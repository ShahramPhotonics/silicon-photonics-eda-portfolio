...
Parameters : Period, duty, chirp, sinusoid or rect sidewall
Stack      : Phase-matched grating generator
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def bragg_period(neff=2.44, wavelength_um=1.55):
    return wavelength_um / (2 * neff)
def build_layout(n_periods=80, duty=0.5, chirp=0.0, corrugation=0.04):
    period = bragg_period()
    rects = []
    x = 0.0
    w0 = 0.50
    for i in range(n_periods):
        p = period * (1.0 + chirp * (i / n_periods - 0.5))
        w = w0 + (corrugation if i % 2 == 0 else -corrugation)
        rects.append((x, -w / 2, x + duty * p, w / 2))
        x += p
    Path("layout").mkdir(exist_ok=True)
    Path("layout/bragg.json").write_text(json.dumps({"period_um": period, "n": n_periods}, indent=2))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="BRAGG")
