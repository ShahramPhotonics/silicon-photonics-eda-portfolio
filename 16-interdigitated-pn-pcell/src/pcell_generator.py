#!/usr/bin/env python3
"""Electro-Optic Phase Shifter with Interdigitated PN Junctions
Technology : High-speed carrier-depletion Si modulators
Materials  : Si 220 nm, P+/P++/N+/N++ implants, silicide, contacts
Parameters : Finger pitch, doping overlap, metal rails
Stack      : Multi-layer implant PCell
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(length=200.0, pitch=0.6, finger=0.25, wg=0.50):
    rects = [(-2, -wg / 2, length + 2, wg / 2)]  # waveguide
    x = 0.0
    sign = 1
    while x < length:
        y0 = sign * wg / 2
        y1 = sign * (wg / 2 + 1.2)
        rects.append((x, min(y0, y1), x + finger, max(y0, y1)))
        x += pitch
        sign *= -1
    # metal rails
    rects.append((0, 2.0, length, 2.4))
    rects.append((0, -2.4, length, -2.0))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="PN_ID")
