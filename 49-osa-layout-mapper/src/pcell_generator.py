...
Parameters : Match IDs to coordinates, pass/fail heatmap overlay
Stack      : CSV join + KLayout overlay
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def map_spectra(csv_text, sites):
    # csv: id,lambda_nm,T
    rows = [ln.split(",") for ln in csv_text.strip().splitlines()[1:]]
    by_id = {}
    for i, lam, T in rows:
        by_id.setdefault(i, []).append((float(lam), float(T)))
    heat = {}
    for sid, (x, y) in sites.items():
        spec = by_id.get(sid, [])
        ok = any(abs(lam - 1550) < 2 and T > 0.5 for lam, T in spec)
        heat[sid] = {"xy": (x, y), "pass": ok}
    return heat
def build_layout():
    csv_text = "id,lambda_nm,T\\na,1550,0.82\\nb,1550,0.21\\nc,1550,0.67\\n"
    sites = {"a": (0.0, 0.0), "b": (20.0, 0.0), "c": (40.0, 0.0)}
    heat = map_spectra(csv_text, sites)
    Path("layout").mkdir(exist_ok=True)
    Path("layout/heatmap.json").write_text(json.dumps(heat, indent=2))
    rects = []
    for sid, info in heat.items():
        x, y = info["xy"]
        s = 3 if info["pass"] else 1.2
...
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="OSA")
