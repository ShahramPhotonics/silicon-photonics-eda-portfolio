...
Parameters : Sub-100 nm lines, filleted inner corners, contact leads
Stack      : High-aspect meander PCell
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(n_turns=12, pitch=0.20, width=0.08, length=8.0):
    rects = []
    y = 0.0
    for i in range(n_turns):
        rects.append((0.0, y, length, y + width))
        if i < n_turns - 1:
            cap_x = length if i % 2 == 0 else 0.0
            rects.append((cap_x - width / 2, y, cap_x + width / 2, y + pitch))
        y += pitch
    # contact pads
    rects.append((-2.0, -1.0, 0.2, 1.0))
    rects.append((length - 0.2, y - 1.0, length + 2.0, y + 1.0))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="SSPD")
