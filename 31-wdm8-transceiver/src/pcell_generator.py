...
Parameters : 8 rings, incremental radii, bus + pads, OASIS out
Stack      : Hierarchical scripted PIC
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(n=8, r0=10.0, dr=0.08, gap=0.20):
    rects = [(-4, -0.25, n * 24 + 4, 0.25)]  # bus
    for i in range(n):
        cx = 12 + i * 24
        r = r0 + i * dr
        rects.append((cx - r, 0.25 + gap, cx + r, 0.25 + gap + 2 * r))
        rects.append((cx - 6, 0.25 + gap + 2 * r + 4, cx + 6, 0.25 + gap + 2 * r + 16))  # pad
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="WDM8")
