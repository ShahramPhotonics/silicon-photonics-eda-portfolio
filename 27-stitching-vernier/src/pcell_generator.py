#!/usr/bin/env python3
"""EBL Beam-Drift Calibration and Stitching Test Patterns
Technology : EBL tool calibration
Materials  : Silicon substrate with Cr/Au marks
Parameters : Vernier scales, interlocking grids at field edges
Stack      : Stitch-error metrology generator
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def vernier(x0, y0, p1=0.20, p2=0.22, n=20):
    rects = []
    for i in range(n):
        rects.append((x0 + i * p1, y0, x0 + i * p1 + 0.06, y0 + 1.2))
        rects.append((x0 + i * p2, y0 + 1.6, x0 + i * p2 + 0.06, y0 + 2.8))
    return rects
def build_layout():
    rects = []
    for k in range(4):
        rects.extend(vernier(k * 8.0, 0.0))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="VERNIER")
