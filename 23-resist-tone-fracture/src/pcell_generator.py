...
Parameters : Boolean invert, tone reverse, bias
Stack      : klayout.db.Region booleans
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def invert_rects(rects, bbox, bias=0.02):
    """Very small boolean invert: emit bbox frame as the 'negative' tone."""
    x0, y0, x1, y1 = bbox
    frame = [
        (x0 - bias, y0 - bias, x1 + bias, y0),
        (x0 - bias, y1, x1 + bias, y1 + bias),
        (x0 - bias, y0, x0, y1),
        (x1, y0, x1 + bias, y1),
    ]
    # positive-tone biased copy
    pos = [(a - bias, b - bias, c + bias, d + bias) for a, b, c, d in rects]
    return frame + pos
def build_layout():
    shapes = [(0, 0, 4, 0.5), (0, 2, 4, 2.5), (1.5, 0, 2.0, 2.5)]
    return invert_rects(shapes, (-1, -1, 5, 3.5), bias=0.03)
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="TONE")
