#!/usr/bin/env python3
"""4-Channel On-Chip CWDM Demultiplexer (Echelle Grating)
Technology : Coarse WDM receiver
Materials  : SOI 220 nm or 300 nm
Parameters : Rowland circle, IO waveguide array, curved facets
Stack      : Analytic Rowland geometry
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def rowland_facets(R=80.0, n_facets=24, m=8, d=0.8):
    rects = []
    for i in range(n_facets):
        ang = math.radians(-12 + 24 * i / (n_facets - 1))
        x = R * math.sin(ang)
        y = R * (1 - math.cos(ang))
        rects.append((x - d / 2, y - 0.15, x + d / 2, y + 0.15))
    # input / 4 outputs on Rowland circle 2R
    rects.append((-2, -2 * R + R, 2, -2 * R + R + 0.6))
    for k in range(4):
        rects.append((12 + k * 4, -2 * R + R, 12 + k * 4 + 0.6, -2 * R + R + 6))
    return rects
def build_layout():
    return rowland_facets()
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="ECHELLE")
