#!/usr/bin/env python3
"""OASIS vs GDSII Compression and Conversion Benchmark
Technology : Tape-out data optimization
Materials  : Large-scale PIC layouts (>1e8 polygons)
Parameters : C-shaped repetition, size and load-time log
Stack      : Conversion pipeline + CSV benchmark
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def synthetic_array(n=200):
    return [(i * 0.2, 0.0, i * 0.2 + 0.1, 0.4) for i in range(n)]
def build_layout():
    rects = synthetic_array()
    raw = 28 * len(rects)  # naive GDS bytes/poly stand-in
    oasis = raw * 0.18     # C-block repetition estimate
    Path("layout").mkdir(exist_ok=True)
    Path("layout/benchmark.csv").write_text(
        "format,bytes,n_poly\\nGDSII,%d,%d\\nOASIS,%d,%d\\n" % (raw, len(rects), int(oasis), len(rects))
    )
    return rects[:40]
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="BENCH")
