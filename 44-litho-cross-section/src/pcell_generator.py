...
Parameters : 2D cut, aerial image, predicted sidewall angle
Stack      : Aerial-image + etch predictor
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def aerial(width_um=0.50, na=0.93, wl_um=0.193, n=256):
    x = np.linspace(-1.0, 1.0, n)
    mask = (np.abs(x) <= width_um / 2).astype(float)
    # incoherent cutoff ~ NA/wl
    k = np.fft.fftfreq(n, d=x[1] - x[0])
    H = (np.abs(k) <= NA_cutoff(na, wl_um)).astype(float)
    img = np.fft.ifft(np.fft.fft(mask) * H).real
    img = np.clip(img / img.max(), 0, 1)
    return x, img
def NA_cutoff(na, wl):
    return na / wl
def sidewall(x, img, thresh=0.3):
    edge = np.where(img > thresh)[0]
    if len(edge) < 4:
        return 90.0
    # fake z from intensity slope
    slope = abs((img[edge[0] + 1] - img[edge[0]]) / (x[1] - x[0]))
    return float(math.degrees(math.atan(1.0 / (slope + 1e-6))))
def build_layout():
    x, img = aerial()
    ang = sidewall(x, img)
    Path("layout").mkdir(exist_ok=True)
    np.savetxt("layout/aerial.csv", np.c_[x, img], delimiter=",")
    Path("layout/sidewall.json").write_text(json.dumps({"deg": ang}, indent=2))
    return [(-0.25, -0.11, 0.25, 0.11)]
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="LITHO")
