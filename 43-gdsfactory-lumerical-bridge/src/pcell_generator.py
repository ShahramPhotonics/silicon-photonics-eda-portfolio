...
Parameters : Export 3D, sweep, save S-params
Stack      : lumapi bridge with stub backend
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def export_fdtd_script(gds="layout/output.gds", wl=1.55):
    return f"""
import lumapi
fdtd = lumapi.FDTD(hide=True)
fdtd.load("{gds}")
fdtd.addfdtd()
fdtd.set("wavelength start", {wl-0.04}e-6)
fdtd.set("wavelength stop", {wl+0.04}e-6)
fdtd.run()
S = fdtd.getresult("monitor", "T")
fdtd.close()
"""
def stub_sparams():
    wl = np.linspace(1.50, 1.60, 21)
    t = np.exp(-((wl - 1.55) / 0.04) ** 2)
    return np.c_[wl, t]
def build_layout():
    Path("layout").mkdir(exist_ok=True)
    Path("layout/run_fdtd.py").write_text(export_fdtd_script())
    np.savetxt("layout/sparams_stub.csv", stub_sparams(), delimiter=",", header="um,T", comments="")
    return [(0, -0.25, 8, 0.25)]
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="FDTD_BRIDGE")
