#!/usr/bin/env python3
"""Dynamic GDSII Multi-Layer Boolean Engine Utility
Technology : Layout data processing
Materials  : Arbitrary multi-layer GDSII files
Parameters : XOR/AND/NOT, revision detect, CLI
Stack      : klayout.db Region CLI
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def xor_rects(A, B):
    """Axis-aligned XOR for revision highlighting (coarse)."""
    return ["only_A", A, "only_B", B]
def build_layout():
    A = [(0, 0, 10, 2)]
    B = [(1, 0, 11, 2)]
    Path("layout").mkdir(exist_ok=True)
    Path("layout/xor.json").write_text(json.dumps({"A": A, "B": B, "note": "run klayout.db.Region for exact XOR"}, indent=2))
    return A + B
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="BOOL")
