#!/usr/bin/env python3
"""Integrated Photonic Gyroscope Sagnac Loop
Technology : Optical sensing
Materials  : Ultra-low loss Si3N4 <0.1 dB/cm
Parameters : 2 m path packed into 10 mm^2 multi-pass spiral
Stack      : Archimedean / multi-pass spiral
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def spiral(length_m=2.0, pitch=0.008, w=0.0018):
    """Pack L meters of 1.8 um SiN into ~10 mm^2."""
    ...
    x = y = 0.0
    ang = 0.0
    while acc < length_um and r < 1600:
        step = 8.0
        ang += step / r
        r += pitch * (step / (2 * math.pi * r + 1e-9)) * 40
        nx, ny = r * math.cos(ang), r * math.sin(ang)
        rects.append((min(x, nx) - w / 2, min(y, ny) - w / 2,
                      max(x, nx) + w / 2, max(y, ny) + w / 2))
        acc += math.hypot(nx - x, ny - y)
        x, y = nx, ny
        if len(rects) > 4000:
            break
    Path("layout").mkdir(exist_ok=True)
    Path("layout/path.json").write_text(json.dumps({"L_um": acc, "n": len(rects)}, indent=2))
    return rects[:: max(1, len(rects) // 400)]
def build_layout():
    return spiral()
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="SAGNAC")
