#!/usr/bin/env python3
"""Automated Optical Connectivity Checker and Pin Snapper
Technology : Physical layout verification
Materials  : Photonic port definitions
Parameters : 1 nm snap tolerance, disconnected pin report
Stack      : Port scanner + log
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def snap_ports(ports, grid_nm=1.0):
    s = 1000.0 / grid_nm
    out = []
    for name, x, y in ports:
        out.append((name, round(x * s) / s, round(y * s) / s))
    return out
def disconnected(ports, traces, tol=0.001):
    bad = []
    for name, x, y in ports:
        ok = any(abs(x - a) <= tol and abs(y - b) <= tol or abs(x - c) <= tol and abs(y - d) <= tol
                 for a, b, c, d in traces)
        if not ok:
            bad.append(name)
    return bad
def build_layout():
    ports = [("in", 0.0004, 0.0), ("out", 10.0, 0.0006), ("nc", 4.0, 3.0)]
    traces = [(0.0, 0.0, 10.0, 0.0)]
    snapped = snap_ports(ports)
    bad = disconnected(snapped, traces)
    Path("layout").mkdir(exist_ok=True)
    Path("layout/pins.json").write_text(json.dumps({"snapped": snapped, "open": bad}, indent=2))
    return traces + [(-0.1, -0.1, 0.1, 0.1), (9.9, -0.1, 10.1, 0.1)]
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="PINS")
