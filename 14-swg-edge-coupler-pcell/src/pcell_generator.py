#!/usr/bin/env python3
"""Sub-Wavelength Grating Edge Coupler PCell
Technology : Edge coupling / packaging
Materials  : Oxide-overclad Si taper + SiN overlay
Parameters : Spot-size conversion to 10.5 um MFD fiber
Stack      : Inverse taper + SWG pitch PCell
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(L=40.0, pitch=0.22, duty0=0.85, duty1=0.25, w0=0.50, w1=0.14):
    n = int(L / pitch)
    rects = []
    x = 0.0
    for i in range(n):
        t = i / max(n - 1, 1)
        duty = duty0 + (duty1 - duty0) * t
        w = w0 + (w1 - w0) * t
        sl = duty * pitch
        rects.append((x, -w / 2, x + sl, w / 2))
        x += pitch
    # SiN overlay pad near the facet
    rects.append((L - 6.0, -1.2, L + 2.0, 1.2))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="SWG_EDGE")
