...
Parameters : Coupler gaps, grating lines, taper tips
Stack      : Feature scan + labeled metrology boxes
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def critical_sites(rects):
    sites = []
    for x0, y0, x1, y1 in rects:
        w, h = x1 - x0, y1 - y0
        if min(w, h) < 0.25:
            sites.append(((x0 + x1) / 2, (y0 + y1) / 2))
    return sites
def build_layout():
    devices = [(0, 0, 8, 0.5), (10, 0, 10.12, 4.0), (14, 0, 18, 0.08)]
    marks = []
    s = 0.4
    for x, y in critical_sites(devices):
        marks.append((x - s, y - s, x + s, y + s))
...
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="CD_SEM")
