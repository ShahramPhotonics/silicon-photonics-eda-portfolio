...
Parameters : Loss spirals, ring Q, CTL contacts, alignment keys
Stack      : Labeled TEG packer
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def paperclip(x, y, n=6, L=40.0, p=4.0):
    rects = []
    for i in range(n):
        yy = y + i * p
        rects.append((x, yy, x + L, yy + 0.5))
        if i < n - 1:
            rects.append((x + L - 0.5, yy, x + L, yy + p + 0.5))
    return rects
def build_layout():
    rects = []
    rects.extend(paperclip(0, 0))
    rects.append((60, 4, 80, 24))  # ring monitor bbox
    rects.append((90, 0, 96, 6))   # CTL
    rects.append((110, 0, 112, 8))
    rects.append((108, 3.5, 114, 4.5))  # alignment cross
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="TEG")
