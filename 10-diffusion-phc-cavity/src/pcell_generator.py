#!/usr/bin/env python3
"""Diffusion Model for Photonic Crystal Cavity Inverse Design
Technology : Photonic crystals (PhC)
Materials  : Suspended InP membrane 250 nm
Parameters : L3 hole displacement profiles, target resonance
Stack      : PyTorch DDPM + hole array GDS
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def l3_displacements(a_um=0.42, r_um=0.12, n_rows=7, n_cols=15):
    """Deterministic L3 cavity with outward hole shift of first neighbors."""
    holes = []
    mid_r, mid_c = n_rows // 2, n_cols // 2
    for i in range(n_rows):
        for j in range(n_cols):
            # triangular lattice
            x = j * a_um + (0.5 * a_um if i % 2 else 0.0)
            y = i * a_um * math.sqrt(3) / 2
            if i == mid_r and abs(j - mid_c) <= 1:
                continue  # three missing holes = L3
            dx = 0.0
            if i == mid_r and abs(j - mid_c) == 2:
                dx = 0.15 * a_um * math.copysign(1, j - mid_c)
            holes.append((x + dx, y, r_um))
    return holes
def build_layout():
    holes = l3_displacements()
    # represent each hole as a square of side ~ 2r (GDS fallback has no circles)
    rects = []
    for x, y, r in holes:
...
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="L3_PHC")
