...
Parameters : 500 x 500 um fields, global/local marks, CSV log
Stack      : BBox tiling + marker cells
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def fields(width_um=2500, height_um=2000, field=500.0):
    nx, ny = math.ceil(width_um / field), math.ceil(height_um / field)
    coords = []
    for iy in range(ny):
        for ix in range(nx):
            coords.append((ix * field, iy * field, field, field))
    return coords
def cross(cx, cy, arm=8.0, t=0.6):
    return [
        (cx - arm, cy - t / 2, cx + arm, cy + t / 2),
        (cx - t / 2, cy - arm, cx + t / 2, cy + arm),
    ]
def build_layout():
    rects = []
    rows = []
    for i, (x, y, w, h) in enumerate(fields()):
        rects.extend(cross(x, y))
        rects.extend(cross(x + w, y + h))
        rows.append({"field": i, "x_um": x, "y_um": y})
    Path("layout").mkdir(exist_ok=True)
    with Path("layout/fields.csv").open("w", newline="") as fh:
        wr = csv.DictWriter(fh, fieldnames=["field", "x_um", "y_um"])
        wr.writeheader()
        wr.writerows(rows)
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="WF_MARKS")
