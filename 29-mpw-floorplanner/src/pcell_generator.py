#!/usr/bin/env python3
"""Multi-User Wafer Layout Floorplanner and Multiplexer
Technology : Multi-project wafer tape-out
Materials  : 100 mm wafer / 5 mm x 5 mm dies
Parameters : Read many GDS, dicing streets, multiplex mask
Stack      : Packing + street frames
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def pack(n_users=8, die=5.0, street=0.1, wafer=100.0):
    pitch = die + street
    n = int((wafer * 0.6) // pitch)
    cells = []
    k = 0
    for iy in range(n):
        for ix in range(n):
            if k >= n_users:
                break
            cells.append((ix * pitch, iy * pitch, ix * pitch + die, iy * pitch + die))
            k += 1
    return cells
def build_layout():
    dies = pack()
    Path("layout").mkdir(exist_ok=True)
    Path("layout/dies.json").write_text(json.dumps({"n": len(dies), "dies": dies}, indent=2))
    return dies
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="MPW")
