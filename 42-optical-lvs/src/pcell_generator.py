#!/usr/bin/env python3
"""Optical Layout Versus Schematic Netlist Extractor
Technology : Automated PIC verification
Materials  : Multi-layer waveguide and electrical layouts
Parameters : Port scan, path length, SPICE/INTERCONNECT netlist
Stack      : Connectivity tracer
Dual-language note
------------------
...
def extract(segments):
    """segments: list of (name, x0,y0,x1,y1) waveguides."""
    nets = []
    for name, x0, y0, x1, y1 in segments:
        L = math.hypot(x1 - x0, y1 - y0)
        nets.append({"inst": name, "length_um": L, "neff": 2.44,
                     "phase": 2 * math.pi * 2.44 * L / 1.55})
    return nets
def spice(nets):
    lines = ["* optical LVS netlist", ".subckt PIC in out"]
    for i, n in enumerate(nets):
        lines.append(f"R{i} n{i} n{i+1} {n['length_um']*10:.3f}")
    lines += [".ends", ".end"]
    return "\\n".join(lines)
def build_layout():
    segs = [("wg_in", 0, 0, 20, 0), ("arm", 20, 0, 20, 12), ("wg_out", 20, 12, 40, 12)]
    nets = extract(segs)
    Path("layout").mkdir(exist_ok=True)
    Path("layout/circuit.sp").write_text(spice(nets))
    Path("layout/nets.json").write_text(json.dumps(nets, indent=2))
    return [(s[1], s[2] - 0.25, s[3], s[4] + 0.25) for s in segs]
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="LVS")
