#!/usr/bin/env python3
"""Automated Electrical Rule Checker for Photonic Heaters
Technology : Thermo-optic circuit verification
Materials  : TiN / aluminum heater layers
Parameters : Opens, metal-WG clearance, current-density width
Stack      : KLayout ERC script
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def erc(metal, waveguides, I_ma=8.0, t_um=0.12, Jmax=5.0, clear=1.0):
    errors = []
    for x0, y0, x1, y1 in metal:
        w = min(x1 - x0, y1 - y0)
        J = I_ma / max(w * t_um, 1e-9)
        if J > Jmax:
            errors.append(("Jmax", (x0, y0, x1, y1), J))
        for a, b, c, d in waveguides:
            dx = max(0, max(a - x1, x0 - c))
            dy = max(0, max(b - y1, y0 - d))
            if math.hypot(dx, dy) < clear and dx * dy == 0:
                errors.append(("clearance", (x0, y0, x1, y1), math.hypot(dx, dy)))
    return errors
def build_layout():
    metal = [(0, 2.0, 40, 2.3)]
    wg = [(0, 0, 40, 0.5)]
    Path("layout").mkdir(exist_ok=True)
    Path("layout/erc.json").write_text(json.dumps(erc(metal, wg), indent=2))
    return metal + wg
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="ERC")
