#!/usr/bin/env python3
"""Quantum Key Distribution Encoder Circuit
Technology : Quantum integrated photonics
Materials  : Silica-on-silicon / thin-film LiNbO3
Parameters : Asymmetric MZIs, state prep, SPD interfaces, trenches
Stack      : QKD encoder floorplan
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout():
    # asymmetric MZI + two phase shifters + detector pads + trenches
    rects = [
        (0, 0, 40, 0.8),
        (0, 12, 52, 12.8),
        (40, 0, 40.8, 12.8),
...
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="QKD")
