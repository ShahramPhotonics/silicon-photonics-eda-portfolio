#!/usr/bin/env python3
"""Optomechanical Crystal Cavity Transducer Layout
Technology : Cavity optomechanics
Materials  : Suspended Si membrane 220 nm, HF undercut
Parameters : 1D/2D nanobeam, tapered hole defect, release windows
Stack      : Nanobeam + etch-release
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(a=0.35, n=21):
    rects = [(-1.0, -0.22, n * a + 1.0, 0.22)]  # beam
    mid = n // 2
    for i in range(n):
        if i == mid:
            continue
        # taper hole size toward defect
        r = 0.10 + 0.03 * abs(i - mid) / mid
        x = i * a
        rects.append((x - r, -r, x + r, r))
    # HF release windows
    rects.append((-2.0, -3.0, n * a + 2.0, -1.2))
    rects.append((-2.0, 1.2, n * a + 2.0, 3.0))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="OMC")
