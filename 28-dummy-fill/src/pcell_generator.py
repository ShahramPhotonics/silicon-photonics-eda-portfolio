#!/usr/bin/env python3
"""Pattern Density Uniformity Filler (Dummy Fill)
Technology : CMP and EBL load balancing
Materials  : Dummy Si tiles in cladding
Parameters : Density 30-50%, keep-out around waveguides
Stack      : Region tiling with exclusion
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(area=(0, 0, 40, 40), keepout=((8, 18, 32, 22),), pitch=4.0, tile=1.6):
    rects = []
    x0, y0, x1, y1 = area
    y = y0
    while y < y1:
        ...
        while x < x1:
            ...
            blocked = any(a <= cx <= c and b <= cy <= d for a, b, c, d in keepout)
            if not blocked:
                rects.append((x, y, x + tile, y + tile))
            x += pitch
        y += pitch
    # waveguide keep-out visualization
    rects.extend(list(keepout))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="DUMMY")
