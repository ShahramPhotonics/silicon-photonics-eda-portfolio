#!/usr/bin/env python3
"""Reconfigurable Ring-Resonator Optical Delay Line
Technology : True time delay networks
Materials  : Low-loss Si3N4 800 nm core
Parameters : Double-ring loaded MZIs, heaters, bus spacing
Stack      : Cascaded delay generator
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def build_layout(n_stages=4, r=12.0):
    rects = [(-6, -0.4, n_stages * 40 + 6, 0.4)]
    for i in range(n_stages):
        x = 10 + i * 40
        rects.append((x - r, 1.0, x + r, 1.0 + 2 * r))
        rects.append((x - r + 4, 1.2, x + r - 4, 1.2 + 2 * r - 0.4))
        rects.append((x - 4, 1.0 + 2 * r + 2, x + 4, 1.0 + 2 * r + 10))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="ODL")
