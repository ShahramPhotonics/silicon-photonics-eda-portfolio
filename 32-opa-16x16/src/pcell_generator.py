...
Parameters : Binary splitter tree, N x N antennas, electrical routing
Stack      : Hierarchical OPA builder
Dual-language note
------------------
...
def write_gds_rects(path: Path, rects_um, layer=1, datatype=0, cell="TOP"):
    ...
    return path
def splitter_tree(depth=4, pitch=12.0):
    rects = []
    # binary tree along x
    x = 0.0
    for d in range(depth + 1):
        n = 2 ** d
        for i in range(n):
            y = (i - (n - 1) / 2) * pitch * (2 ** (depth - d)) * 0.25
            rects.append((x, y - 0.2, x + 8.0, y + 0.2))
        x += 10.0
    return rects, x
def build_layout(n=16):
    rects, x0 = splitter_tree(int(math.log2(n)))
    for i in range(n):
        for j in range(n):
            rects.append((x0 + j * 4.0, (i - n / 2) * 4.0, x0 + j * 4.0 + 1.6, (i - n / 2) * 4.0 + 0.4))
    return rects
def main():
    ...
    rects = build_layout()
    write_gds_rects(args.out, rects, cell="OPA16")
